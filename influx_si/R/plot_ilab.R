plot_mti=function(ti, x, m=NULL, ...) {
   ...
   # m can have repeated measurements for a same x
#browser()
   nmx=rownames(x)
   nmm=rownames(m)
   nb_curve=NROW(x)
   if (length(nmx) == 0L) {
      nmx=seq_len(nb_curve)
      nm_leg=as.character(seq_len(nb_curve))
   } else {
      # strip the ftbl row number
      nmx=sub(":[-0-9]+$", "", nmx)
      # make names look like M_0, M_1 etc.
      if (length((nms <- strsplit(nmx, ":", fixed=TRUE))[[1L]]) > 2L) {
   ...
   legend("topright", legend=nm_leg, lty=seq_len(nb_curve), col=seq_len(nb_curve), lwd=2, cex=0.75, bg=rgb(0.97,0.97,0.97, 0.75))
}
...
#browser()
      if (length(nm_selm) > 0L) {
         ...
         for (metf in nmf) {
            ...
            isimu=unique(apply(strsplitlim(isim, ":", fixed=TRUE, mat=TRUE)[,1L:4L,drop=FALSE], 1L, paste0, collapse=":"))
            iu=which(startsWith(isim, isimu))[seq_along(isimu)]
            ...
            mainlab=if (is.na(fr) || fr == paste(seq_len(metlen), collapse=",") || fr == sprintf("1~%d", metlen))
               met else
               sprintf("%s:%s", met, fr)
            plot_mti(tifull[[iexp]][-1L], usmf[isim[iu],,drop=FALSE], me[i,,drop=FALSE], main=mainlab, ylim=0:1)
         }
      } else {
...
#browser()
      if (exists("mid") && length(mid)) {
         ...
         if (length(nm_sell) > 0) {
            ...
            nmf=unique(apply(strsplitlim(nm_sell, ":", fixed=TRUE, mat=TRUE)[,1:3,drop=FALSE], 1L, paste0, sep="", collapse=":"))
            for (metf in nmf) {
               ...
               if (length(isim) == 0L)
                  next
               isimu=unique(apply(strsplitlim(isim, ":", fixed=TRUE, mat=TRUE)[,1L:3L,drop=FALSE], 1L, paste0, collapse=":"))
               iu=which(startsWith(isim, isimu))[seq_along(isimu)]
               ...
               plot_lti(tifull[[iexp]][-1L], usmf[isim[iu],,drop=FALSE], me[i,,drop=FALSE], main=mainlab, ylim=0:1)
            }
         }
      }
