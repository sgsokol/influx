...
def main(argv=sys.argv[1:]):
    ...
    invcomp={">=": "<=", "=>": "<=", "<=": ">=", "=<": ">="}
    # parse options
    opts,args=getopt.getopt(argv, "hi", ["help", "force", "out="])
    for o,a in opts:
        if o in ("-h", "--help"):
            usage()
            return 0
    ...
    if not args:
        usage()
        raise Exception("Expecting ftbl file name")
    ...
    # get non reversible fluxes
    nonrev=set([d["NAME"] for d in ftbl.get("FLUXES", {}).get("XCH", []) if d["FCD"] == "C" and float(d["VALUE(F/C)"]) == 0])
    bsl="\\" # backslash
    ...
    for suff in (".netw", ".linp", ".miso", ".mflux", ".mmet", ".tvar", ".cnstr", ".opt"):
        ...
        if suff == ".netw":
            ...
            for rnm,reac in ftbl["long_reac"].items():
                ...
                s=f"{rnm}:\t{dloc['left']} {'<' if rnm not in nonrev else ''}-> {dloc['right']}\n"
                res += s
        elif suff == ".linp":
            # label input
            ...
            for nx in ("NET", "XCH"):
                for d in ftbl["FLUXES"][nx]:
                    if nx == "XCH" and d["FCD"] == "C" and float(d['VALUE(F/C)']) == 0.:
                        continue
                    res += f"\t\t{d['NAME']}\t{nx}\t{d['FCD']}\t{d['VALUE(F/C)']}\n"
            for d in ftbl.get("METABOLITE_POOLS", []):
                fcd="F" if d['META_SIZE'] < 0 else "C"
                res += f"\t\t{d['META_NAME']}\tMETAB\t{fcd}\t{abs(d['META_SIZE'])}\n"
        elif suff == ".cnstr":
